...
do while .not. eof ()
   ...
   if .not. [HDR] $ yfld .and. .not. [TLR] $ yfld    && skip header/trailer
      ...
      l_fmake = substr(field,53,7)
      l_fstyle = substr(field,60,10)
      l_fstyle = substr(strtran(alltrim(l_fstyle),"/",""),1,2)
      l_fmsrp = substr(field,70,6)
      l_fref = substr(field,76,3)+[ ]+substr(field,79,7)+[ ]+substr(field,86,3)
      l_frectyp = val(substr(field,89,2))     && type = 4,8,9,15
      * l_fdate = ctod(substr(field,93,2)+"/"+substr(field,95,2)+"/20"+substr(field,91,2))
      * l_fclerk = substr(field,97,1)
      l_fcontrol = substr(field,100,4)        && pending: bill knight
      l_freg = val(substr(field,104,7))
      l_ftitle = val(substr(field,111,7))
      l_ftfee = val(substr(field,118,7))

      * --------10.05.07------------------
      ylline1 = substr(l_fref,1,11)
      ylline2 = substr(field,125,25)
      ylline3 = substr(field,150,30)
      ylline4 = substr(field,180,30)
      if empty(ylline4)
         ylline4 = alltrim(substr(field,210,14))+",  "+ ;
                   substr(field,224,2)+" "+substr(field,226,9) 
         ylline5 = ""
      else
         ylline5 = alltrim(substr(field,210,14))+",  "+ ;
                   substr(field,224,2)+" "+substr(field,226,9) 
      endif
      * ----------------------------------
